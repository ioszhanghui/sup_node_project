...
var queryID = "SELECT * FROM user_info_table u where u.open_id=?"//登录用户查询
var loginSaveSql = 'insert into user_info_table(open_id,session_key,nickName,gender,avatarUrl) values(?,?,?,?,?)';//保存登录的用户id
//优惠券列表
var discount_listSql ="SELECT user_discount_relation.open_id,user_discount_relation.discount_id,user_discount_relation.is_get,user_discount_relation.is_use,sup_discount_coupon.discount_amount,sup_discount_coupon.discount_title,sup_discount_coupon.discount_reduce_amount,sup_discount_coupon.discount_outtime,sup_discount_coupon.discount_create_time,user_discount_relation.discount_code FROM user_discount_relation RIGHT JOIN sup_discount_coupon ON user_discount_relation.discount_id = sup_discount_coupon.id WHERE user_discount_relation.open_id=?ORDER BY sup_discount_coupon.discount_create_time DESC";//查询已经领取的优惠券
